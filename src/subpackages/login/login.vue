<template>
            ...
            <view class="input-group">
              ...
              <view class="input-wrapper">
                <input 
                  ...
                  maxlength="11"
                />
                <view v-if="form.phone" class="input-clear" @click="clearPhone">
                  <text>✕</text>
                </view>
              </view>
            </view>
  ...
  import { loginByPassword,getCurrentUser } from '@/services/apiService';
  ...
  export default defineComponent({
    ...
    computed: {
      // 表单验证
      isFormValid(): boolean {
        // return validatePhone(this.form.phone) && validatePassword(this.form.password);
        return true;
      }
    },
    ...
    methods: {
      ...
      loadSavedAccount() {
        try {
          ...
          if (savedAccount) {
            this.form.phone = savedAccount.phone || '';
      ...
      async handleLogin() {
        ...
        try {
          ...
          if (result.success) {
            ...
            showToast(result.message || '登录失败', 'none');
          }
        } catch (error: any) {
